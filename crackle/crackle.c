...
void do_crack(crackle_state_t *state) {
    ...
    int tk = -1;
    unsigned char dev_1[6],dev_2[6];
    char pairing[30];
    char connection_type[30]="Legacy pairing";
    ...
    for (i = 0; i <= state->current_conn; ++i) {
        ...
        if (strategy < 0) {
            ...
            for (j = 0; j < num_errors; ++j){
                printf("   %s\n", errors[j]);
                if( strcmp(errors[j] ,"LE Secure Connections") ==0 ){
                    strcpy(pairing, "Can`t check pairing method");
                    strcpy(connection_type, "LE Secure Connections");
                }
            }
            continue;
        }
        ...
        if (tk >= 0) {
            ...
            printf("\n  !!!\n");
            printf("  TK found: %06d\n", tk);
            //pairing = "Passkey Entry";
            strcpy(pairing, "Passkey Entry");
            if (tk == 0){
                strcpy(pairing, "Just Works");
                //pairing = "Just Works";
    ...
    mysql_init(&mysql);
    if ( !mysql_real_connect(&mysql,HOST,USER,PASSWORD,DB,1236,NULL,0) ) { /*连接*/
            printf("connect mysql fail!!!\n");
            exit(0);
    }
    //sprintf(query, "INSERT INTO bluetooth_report(`device1`, `device2`, `connect_type`, `pairing_type`, `TK`) VALUES('%02x:%02x:%02x:%02x:%02x:%02x','%02x:%02x:%02x:%02x:%02x:%02x','%s','%s','%06d') where;",dev_1[0],dev_1[1],dev_1[2],dev_1[3],dev_1[4],dev_1[5],dev_2[0],dev_2[1],dev_2[2],dev_2[3],dev_2[4],dev_2[5],connection_type,pairing,tk);
    //if(tk!='-00001' && connection_type!='Legacy pairing')
    sprintf(query, "INSERT INTO bluetooth_report(`device1`, `device2`, `connect_type`, `pairing_type`, `TK`) SELECT '%02x:%02x:%02x:%02x:%02x:%02x','%02x:%02x:%02x:%02x:%02x:%02x','%s','%s','%06d' FROM DUAL WHERE NOT EXISTS(SELECT `device1`, `device2` FROM bluetooth_report WHERE `device1` = '%02x:%02x:%02x:%02x:%02x:%02x' and `device2` = '%02x:%02x:%02x:%02x:%02x:%02x');",dev_1[0],dev_1[1],dev_1[2],dev_1[3],dev_1[4],dev_1[5],dev_2[0],dev_2[1],dev_2[2],dev_2[3],dev_2[4],dev_2[5],connection_type,pairing,tk,dev_1[0],dev_1[1],dev_1[2],dev_1[3],dev_1[4],dev_1[5],dev_2[0],dev_2[1],dev_2[2],dev_2[3],dev_2[4],dev_2[5]);
    ...
    mysql_close(&mysql);
    printf("\n%02x:%02x:%02x:%02x:%02x:%02x",dev_1[0],dev_1[1],dev_1[2],dev_1[3],dev_1[4],dev_1[5]);
    printf("\n%02x:%02x:%02x:%02x:%02x:%02x\n",dev_2[0],dev_2[1],dev_2[2],dev_2[3],dev_2[4],dev_2[5]);
    printf("%s",pairing);
}
